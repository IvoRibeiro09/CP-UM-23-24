...
__global__ void PotentialGPU(double *d_a, double *d_r, double *d_Pot);
...
__global__ void PotentialGPU(double *d_a, double *d_r, double *d_Pot) {
    ...
    if (i < d_NCuda - 1) {
        ...
        for (int j = i + 1; j < d_NCuda; j++) {
            ...
            //     d_a_aux[k] += auxrij;
            //     atomicAddDouble(&d_a[j * 3 + k], -auxrij);
            // }
        }
        ...
        // for (int k = 0; k < 3; k++) {
        //     atomicAddDouble(&d_a[i * 3 + k], d_a_aux[k]);
        // }
    }
}
...
void Potential() {
    ...
    PotentialGPU<<<bpg, NUM_THREADS_PER_BLOCK>>>(d_a, d_r, d_Pot);
